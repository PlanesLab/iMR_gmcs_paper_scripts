...
plot_final_gmcs <- ComplexUpset::upset(
  ...
  base_annotations=list(
    ...
    + scale_color_manual(
      values=c('Human1'=colors_final[1], 'Human1-O1'= colors_final[2], 'Human1-O2'= colors_final[3], 'Human1-O3'= colors_final[4], 
               'Human1-D1'=colors_final[5], 'Human1-D2'=colors_final[6],'Human1-D3'=colors_final[7],
               'Human1-T1'=colors_final[8], 'Human1-T2'=colors_final[9],'Human1-T3'=colors_final[10]),
      guide='none'
      # guide_legend(override.aes=list(shape='circle'))
    )
  ),queries=list(
    upset_query(set='Human1', fill=colors_final[1]),
    upset_query(set='Human1-O1', fill= colors_final[2]),
    upset_query(set='Human1-O2', fill= colors_final[3]),
    upset_query(set='Human1-O3', fill= colors_final[4]),
    upset_query(set='Human1-D1', fill=colors_final[5]),
    upset_query(set='Human1-D2', fill=colors_final[6]),
    upset_query(set='Human1-D3', fill=colors_final[7]),
    upset_query(set='Human1-T1', fill=colors_final[8]),
    upset_query(set='Human1-T2', fill=colors_final[9]),
    upset_query(set='Human1-T3', fill=colors_final[10])
...
ggsave(paste0("./plots/SuppFig8.pdf"),
       plot = plot_final_gmcs, device = "pdf", width = 9, height = 6, units = "in", dpi = 300, bg = "white")

function [gmcs, gmcs_time, gmcs_onlynew] = calculateGeneMCS(model_name, model_struct, n_gmcs, max_len_gmcs, varargin)
...
addParameter(p, 'min_len_gmcs', 1, @(x)isnumeric(x)&&isscalar(x));
addParameter(p, 'warmStartGeneMCS', [], @(x)iscell(x)||isempty(x));
...
min_len_gmcs = p.Results.min_len_gmcs;
warmStartGeneMCS = p.Results.warmStartGeneMCS;
...
if exist(G_file, 'file') == 2
    ...
    if exist('G_rxns', 'var')
        assert(isequal(model_struct.rxns, G_rxns), 'G matrix is not the same, different genes in G matrix and in the model.');
    end
%     assert(all(ismember(unique([G_ind{:}]), unique(strtok(model_struct.genes, separate_transcript)))), 'G matrix is not the same, different reaction order in the G matrix and the model.');
else
...
% generate model depending on the KO field
if isempty(KO)
    % ENUMERATE gMCSs
    % Define variables
    var.u = 1:n_mets;
    ...
    A = spalloc(n_cons, n_vars, sum(sum(G~=0)) + sum(sum(S~=0)) + sum(sum(t~=0)) + numel(related)); % memory efficient
    ...
    if n_relations > 0
        ...
        %         end
%         tic
        idx_relations = sub2ind(size(A), cons.relations, var.zp(related(:, 1)));
        A(idx_relations) = -1;
%         toc
%         tic
        idx_relations = sub2ind(size(A), cons.relations, var.zp(related(:, 2)));
        A(idx_relations) = +1;
%         toc
%         tic
%         idx_relations = sub2ind(size(A), [cons.relations; cons.relations], [var.zp(related(:, 1)); var.zp(related(:, 2))]);
%         A(idx_relations(1:length(cons.relations))) = -1;
%         A(idx_relations((1:length(cons.relations)) + length(cons.relations))) = +1;
%         toc
%         clear idx_relations
    end
    % if forceLength == 1
    if max_len_gmcs < length(unique([G_ind{:}])) || min_len_gmcs>1
        % allways consider the maximum number of genes that can be in a gMCS
    ...
    if n_relations > 0
        rhs(cons.relations) = inf;
    end
    if forceLength == 1
        rhs(cons.forceLength) = min_len_gmcs;
    else
        rhs(cons.forceLength) = max_len_gmcs;
    end
    lhs = zeros(n_cons, 1);
    lhs(cons.Ndual, 1) = 0;
    ...
    %     if forceLength == 1
    lhs(cons.forceLength) = min_len_gmcs;
    %     end
...
% with the model created, solve it iteratively %%
...
% Include previously calculated gMCS
if ~isempty(warmStartGeneMCS)
...
%     tic
    ...
    Gind2genes_mat_idx = cellfun(@(x)find(ismember(Gind2genes_genes,x)), G_ind, 'UniformOutput', false);
%     Gind2genes_mat_idx = sub2ind(size(Gind2genes_mat), repelem(1:length(Gind2genes_mat_idx), cellfun(@length, Gind2genes_mat_idx)), [Gind2genes_mat_idx{:}]);
%     Gind2genes_mat(Gind2genes_mat_idx) = 1;
%     toc
    for i = 1:length(G_ind)
        Gind2genes_mat(i,Gind2genes_mat_idx{i}) = 1;
    end
    clear Gind2genes_mat_idx
    ...
    % transform gMCS into lines of A matrix to include (similar to the
    % process done by this function
    A_ws = spalloc(numel(warmStartGeneMCS), size(A,2), numel(size(warmStartGeneMCS)));
    rhs_ws = zeros(size(warmStartGeneMCS));
    lhs_ws = zeros(size(warmStartGeneMCS));
    warmStartGeneMCS_aux = cellfun(@(x)ismember(Gind2genes_genes,x), warmStartGeneMCS, 'UniformOutput', false);
%     warmStartGeneMCS_aux = cellfun(@find, warmStartGeneMCS_aux, 'UniformOutput', false);
    for j = 1:length(warmStartGeneMCS)
%         tmp_gmcs = warmStartGeneMCS{j};
%         n_cons = n_cons+1;
%         sol = sum(Gind2genes_mat(:,ismember(Gind2genes_genes, tmp_gmcs)),2)==1;
        sol = sum(Gind2genes_mat(:,warmStartGeneMCS_aux{j}),2)==1;
        sol = find(sol);
        A_ws( j, var.zp(sol)) = 1;
        rhs_ws(j) = length(sol) - 1; %sum(sol)-1;
        lhs_ws(j) = 0;
    end   
    cplex.Model.A = [cplex.Model.A; A_ws];
    cplex.Model.rhs = [cplex.Model.rhs; rhs_ws];
    cplex.Model.lhs = [cplex.Model.lhs; lhs_ws];
    n_cons = size(cplex.Model.A,1);
    
    % solve once to search for the feasible region
...
while largest_gmcs <= max_len_gmcs && cplex.Model.rhs(cons.forceLength) <= max_len_gmcs && k < n_gmcs
    ...
    if ~isempty(warmStartGeneMCS)
        cplex.feasOpt([],[],[],[]);
        if numel(warmStartGeneMCS)>1000 
            cplex.solve(); 
        end
    end
    cplex.populate();
    ...
    if n_pool ~= 0
        ...
        for j = 1:n_pool
            ...
            % sol2 = sum(Gind2genes_mat(:,ismember(Gind2genes_genes, tmp_gmcs)),2)==1;
    ...
    try save(['tmp' model_name '_FL_' num2str(forceLength) '_' num2str(n_gmcs) '.mat'], 'gmcs', 'gmcs_time'); end

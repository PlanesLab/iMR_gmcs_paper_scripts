function [G, G_ind, related, n_genes_KO, G_time, G_rxns, iMRmodel_info] = buildGmatrix_iMRmodel(model_name, model_struct, regulatoryNetwork, nLayers)
   ...
   n_mcs = 100000000000;
    ...
    for i = 1:length(uniqueGPR)
        ...
        if uniqueGPR(i)~=""
            ...
            target = 'target';
            [gMCSs] = generateModel(RxnFormulas, auxKO_Nodes, target, n_mcs, inputs, 1);
            allgMCSs{i} = gMCSs;
        else
            allgMCSs{i} = {};
        end 
...
final_filename = [pwd filesep 'G_matrices_non_simplified' filesep 'G_' model_name '_' num2str(nLayers) '_layers.mat'];
...
function [gMCSs] = generateModel(RxnFormulas, auxKO_Nodes, target, n_mcs, inputs, forceLength)
